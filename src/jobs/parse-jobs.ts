...
client.defineJob({
  ...
  run: async (payload, io, ctx) => {
    const jobLocations = (
      process.env.JOB_LOCATIONS || "new york,new jersey"
    ).split(",");
    ...
    const job_requirements =
      process.env.JOB_REQUIREMENTS || "no_experience,under_3_years_experience";
    const job_query =
      process.env.JOB_QUERY || "software engineer jobs in PLACEHOLDER usa";
    ...
    if (ignoredDomains.length === 0) {
      ...
      await io.logger.info(
        `Ignoring jobs from domains: ${ignoredDomains.join(", ")}`,
      );
    }
      ...
      try {
        ...
        for (const job of data.data) {
          let ignoreJob = ignoredDomains.some((domain) => {
            ...
            return (
              job.job_apply_link &&
              regex.test(new URL(job.job_apply_link).hostname)
            );
          });
          ...
          if (ignoreJob) {
            ...
            io.logger.info(
              `Ignoring job with apply link: ${job.job_apply_link} for location: ${location} with cache key: ${cacheKey}`,
            );
        ...
        await io.logger.error(
          `Failed to fetch jobs for location: ${location} - ${error}`,
        );
        await logsnag.track({
          channel: "parse-jobs",
    ...
    if (totalStats.totalCreated !== 0) {
      const users = (await getAllUsers()) as {
      ...
      if (users.length > 0) {
        ...
        await io.logger.info(
          "Done sending event to Loops for ParseJobs event to users with ASAP frequency",
        );
      } else {
        await io.logger.warn(
          "No users found to send event to Loops for ParseJobs event",
        );
      }
    } else {
      await io.logger.warn(
        "No jobs created to send event to Loops for ParseJobs event",
      );
    }
    ...
    await logsnag.track({
      ...
      description:
        `Job fetching and storage process summary` + JSON.stringify(totalStats),
      icon: "🔍",
      notify: true,
      tags: {
