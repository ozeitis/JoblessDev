...
const frequencyOptions = {
  asap: {
    label: "ASAP",
    description: `You will receive emails whenever we get new emails (currently every ${Number(asapFrequency) / 3600} hours).`
  },
  daily: {
    label: "Daily",
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          <fieldset disabled={!isLoaded}>
            <legend className="text-base font-medium text-gray-900 dark:text-gray-100">Email Frequency</legend>
            <div className="mt-4 space-y-4">
              {Object.entries(frequencyOptions).map(([key, { label, description }]) => (
                <div key={key} className="flex items-center">
                  <input
                    ...
                    value={key}
                  />
                  <label className="ml-3 block text-sm font-medium text-gray-700 dark:text-gray-300" htmlFor={key}>
